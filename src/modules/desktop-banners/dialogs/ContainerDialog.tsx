...
const ContainerDialog: React.FC = () => {
  ...
  const [container, setContainer] = useState(
    desktopBannerContext?.bannerData?.container
  );
  ...
  const validateContainerData = useCallback(() => {
    if (container?.backgroundType === 'normal') {
      if (!container.backgroundColor || container.backgroundColor[0] !== '#')
        return false;
    }

    if (container?.backgroundType === 'linear-gradient') {
      if (
        !container.backgroundColorLinear01 ||
        container.backgroundColorLinear01[0] !== '#'
      )
        return false;
      if (
        !container.backgroundColorLinear02 ||
        container.backgroundColorLinear02[0] !== '#'
      )
        return false;
    }
  ...
  const backgroundTypes = useMemo(() => {
    return [
      ...
      { name: 'Degrade', value: 'linear-gradient' }
    ];
  }, []);
    ...
    return (
      <>
        <ColorInputPicker
          ...
          onChange={(newColor) =>
            onUpdateInput('backgroundColorLinear01', newColor)
          }
        />
        <ColorInputPicker
          ...
          onChange={(newColor) =>
            onUpdateInput('backgroundColorLinear02', newColor)
          }
        />
      </>
    );
  ...
  return (
    <DefaultDialogContainer>
      ...
      <Form>
        ...
        <RadioGroup
          value={container?.backgroundType}
          onChange={(event) =>
            onUpdateInput('backgroundType', event.target.value)
          }
        >
        ...
        <CustomInput
          ...
          onChange={(event) =>
            onUpdateInput('redirectLink', event.target.value)
          }
          variant="outlined"
        />
