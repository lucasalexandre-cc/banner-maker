...
export default function useValidate(): UseValidateMethods {
  ...
  const getStripErrors = useCallback(
    (stripData: DesktopBannerStripData | undefined): Array<string> => {
      const errors: Array<string> = [];
      if (!stripData) return errors;

      if (!stripData.backgroundColor || stripData.backgroundColor[0] !== '#')
        errors.push('Cor de fundo da faixa é obrigatório');
      if (!stripData.fontColor || stripData.fontColor[0] !== '#')
        errors.push('Cor da fonte da faixa é obrigatório');
  ...
  const getTitleErrors = useCallback(
    (titleData: DesktopBannerTitleData | undefined): Array<string> => {
      const errors: Array<string> = [];
  ...
  const getSubtitleErrors = useCallback(
    (subtitleData: DesktopBannerSubtitleData | undefined): Array<string> => {
      const errors: Array<string> = [];
  ...
  const getButtonErrors = useCallback(
    (buttonData: DesktopBannerButtonData | undefined): Array<string> => {
      const errors: Array<string> = [];
  ...
  const getBannerDataErrors = useCallback(
    (bannerData: DesktopBannerData): Array<string> => {
      const errors: Array<string> = [];
