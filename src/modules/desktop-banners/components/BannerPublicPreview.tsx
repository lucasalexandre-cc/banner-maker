...
const BannerPublicPreview: React.FC = () => {
  ...
  const showTrialReason = useCallback(() => {
    ...
    return userAccess?.type === "with_trial" &&
      userAccess?.trialReason;
  }, [desktopBannerContext]);

  const showSpecificUniversities = useCallback(() => {
    return desktopBannerContext?.bannerData.public.universities?.type === "specifics";
  }, [desktopBannerContext]);

  const getUniversityName = useCallback((universityId) => {
  ...
  return (
    <Container>
      ...
      {showTrialReason() && <Info>Razão do trial: {desktopBannerContext?.bannerData.public.userAccess.trialReason}</Info>}
      <Info>Tipo de universidade: {universitiesTypeName}</Info>
      {showSpecificUniversities() && (
        <>
          {desktopBannerContext?.bannerData.public.universities.ids?.map(universityId => (
